...
import { NutritionAssessmentModule } from './components/nutrition/NutritionAssessmentModule';
...
import { PPGUploadZone } from './components/scanner/PPGUploadZone';
import { WhatToDoNextCard } from './components/results/WhatToDoNextCard';
import { ComprehensiveScreeningSummary } from './components/results/ComprehensiveScreeningSummary';
...
export const COMMON_SYMPTOMS = [
  ...
  { id: 'pale_eyelid', label: 'Pale Palpebral Conjunctiva (Inner Eyelids)', desc: 'Loss of healthy pink/red mucosal coloration' },
...
export function App() {
  // Top navigation view mode: 'scanner' | 'doubt' | 'nutrition'
  const [viewMode, setViewMode] = useState<'scanner' | 'doubt' | 'nutrition'>('scanner');
  ...
  // Camera, Image & PPG Sensor State
  const [cameraRoiRegion, setCameraRoiRegion] = useState<'Palpebral Conjunctiva'>('Palpebral Conjunctiva');
  const [simulatedQuality, setSimulatedQuality] = useState<'GOOD' | 'BAD'>('GOOD');
  const [capturedImage, setCapturedImage] = useState<string | null>(null);
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [ppgFile, setPpgFile] = useState<File | null>(null);
  ...
  const handleRunAnalysis = async () => {
    ...
    const anthropometryData: AnthropometryData = {
      weightKg: category === 'child' ? 14.0 : 55.0,
      heightCm: category === 'child' ? 95.0 : 160.0,
      muacCm: category === 'child' ? 13.5 : 24.0,
    };
    ...
    try {
      ...
      if (selectedSymptoms.length > 0) {
        res.contributingSignals.unshift({
          ...
          description: `Patient reported: ${selectedSymptoms
            .map((s) => COMMON_SYMPTOMS.find((cs) => cs.id === s)?.label)
            .join(', ')}.`,
        });
      }
      ...
      setAnalysisError(err?.message || 'Screening evaluation failed. Please check inputs and retry.');
      setStage('symptoms');
    }
  };
        ...
        <div className="max-w-[980px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#86868b] hidden sm:block">
                Optical Conjunctiva Anemia & MAX30102 PPG Hardware Screening
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isBackendConnected ? (
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 text-amber-800 border border-amber-200 text-[11px] font-medium">
      ...
      <main className="flex-1 max-w-[840px] w-full mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-center p-1 rounded-2xl bg-black/[0.05] border border-black/[0.04] max-w-lg mx-auto">
          ...
          >
            ...
            <span>Multimodal Screening</span>
          </button>
          ...
          >
            <Scale className="w-4 h-4 text-[#00776b]" />
        ...
        {/* VIEW 3: NUTRITION & ANTHROPOMETRIC ASSESSMENT */}
        {viewMode === 'nutrition' && (
          <NutritionAssessmentModule
            initialAgeYears={ageYears}
            initialCategory={category}
            initialGender={sex === 'Male' ? 'Male' : 'Female'}
        ...
        {/* VIEW 1: MULTIMODAL ANEMIA SCREENING WIZARD */}
        {viewMode === 'scanner' && (
          <div className="space-y-6">
            ...
            {stage === 'input' && (
              <div className="bg-white rounded-[32px] p-8 border border-black/[0.06] shadow-sm space-y-6">
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Sunita Devi or ID #1042"
                ...
                <div className="pt-4 flex justify-end">
                  <button
                    onClick={() => setStage('camera')}
                    className="apple-btn-accent px-7 py-3.5 text-[14px] inline-flex items-center gap-2 shadow-sm cursor-pointer"
                  >
                    <span>Proceed to Modality Input</span>
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-[32px] p-8 border border-black/[0.06] shadow-sm space-y-6">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-black/[0.06]">
                    <button
                      onClick={() => setStage('input')}
                      className="apple-btn-secondary px-5 py-3 text-[13px] cursor-pointer"
                    >
                      ← Back
                    </button>
                    ...
                    <button
                      onClick={() => setStage('symptoms')}
                      className="apple-btn-accent px-7 py-3.5 text-[13px] inline-flex items-center gap-2 shadow-sm font-medium cursor-pointer"
                    >
              ...
              <div className="bg-white rounded-[32px] p-8 border border-black/[0.06] shadow-sm space-y-8">
                ...
                {analysisError && (
                  <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3 text-[13px] text-red-900">
                    ...
                    <div className="space-y-1">
                      ...
                      <p>{analysisError}</p>
                      <p className="text-[12px] text-red-700">All entered patient data has been preserved. Please verify backend service connectivity and click &apos;Run Multimodal Analysis&apos; below to retry.</p>
                    </div>
                  </div>
                )}
                ...
                <div className="p-6 rounded-[28px] bg-[#fbfbfd] border border-black/[0.06] space-y-4">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="e.g. 'I feel dizzy after standing' or 'My inner eyelids look pale'"
                    ...
                    <button
                      ...
                      className="apple-btn-secondary px-5 py-3 text-[13px] font-medium disabled:opacity-40 cursor-pointer"
                    >
                      Check Doubt
                    </button>
                  </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-black/[0.06]">
                  ...
                  <button
                    ...
                    className="apple-btn-accent px-8 py-3.5 text-[14px] inline-flex items-center gap-2 shadow-sm font-medium cursor-pointer"
                  >
            ...
            {stage === 'analyzing' && (
              <div className="bg-white rounded-[32px] p-12 border border-black/[0.06] shadow-sm space-y-6 text-center">
                ...
                <div className="space-y-1">
                  <h3 className="text-[22px] font-semibold text-[#1d1d1f]">Evaluating Multimodal Screening Signals</h3>
                  <p className="text-[13px] text-[#86868b]">
                    Executing Image CV, Optical PPG, and Clinical Risk Assessment Engine...
                  </p>
                </div>

                <div className="max-w-md mx-auto space-y-2 text-left text-[12px]">
                  {[
                    { id: 1, name: 'Palpebral Conjunctiva Image Feature Extraction' },
                    { id: 2, name: 'MAX30102 Optical PPG 25Hz Dual-Wavelength Hemoglobin Regression' },
                    { id: 3, name: `WHO 2024 Threshold Matching (${category === 'child' ? 'Pediatric' : sex === 'Male' ? 'Adult Male <13g/dL' : 'Adult Female <12g/dL'})` },
                    { id: 4, name: 'Deterministic WHO Safety Rule & Red Flag Verification' },
                    { id: 5, name: 'Clinical Risk Assessment Triage & Database Persistence' },
                  ].map((s) => (
                    <div
                      key={s.id}
            ...
            {stage === 'result' && screeningResult && (
              <div className="bg-white rounded-[32px] p-8 border border-black/[0.06] shadow-sm space-y-8">
                ...
                {/* Result Header with Patient and Database Metadata */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-black/[0.06] pb-6">
                  <div>
                    <div className="flex items-center gap-2 text-[#00776b] text-[12px] font-semibold mb-1">
                      ...
                      <span>Point-of-Care Multimodal Assessment Complete</span>
                    </div>
                  ...
                  <div className="flex flex-col items-start sm:items-end gap-2">
                    {screeningResult.backendScreeningId ? (
                      <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-[11px] font-mono">
                        ...
                        <span>Screening #{screeningResult.backendScreeningId}</span>
                      </div>
                    ) : (
                      <span className="px-3 py-1 rounded-full bg-[#f5f5f7] text-[#6e6e73] text-[11px] font-mono">
                        Local Enclave Session
                      </span>
                    )}

                    <button
                      onClick={handleStartNewScreening}
                      className="apple-btn-accent px-5 py-2 text-[13px] inline-flex items-center gap-1.5 shadow-sm cursor-pointer"
                    >
                      <Plus className="w-3.5 h-3.5 stroke-[2.5]" />
                ...
                <ComprehensiveScreeningSummary
                  ...
                  beneficiary={{
                    ...
                    trimester: category === 'pregnant' ? trimester : undefined,
                    locationVillage,
                    anganwadiCentreId: 'AWC-1',
                    anemiaRisk: screeningResult.anemiaRisk,
                    nutritionRisk: screeningResult.nutritionRisk,
                    overallPriority: screeningResult.overallPriority,
                    trajectory: screeningResult.trajectory,
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    onClick={handleStartNewScreening}
                    className="apple-btn-primary px-8 py-3.5 text-[14px] shadow-sm font-medium cursor-pointer"
                  >
